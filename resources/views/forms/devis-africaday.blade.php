        ...
        <div class="row">
            <div class="col-lg-12 center-col">
                <div class="area-heading1 mt-6 wow fadeInRight ctk1">
                    <form id="dev-form" method="post" action="{{route('make-reservation')}}">
                        @csrf
                        <h4 class="text-center alt-font text-white mb-2 font-weight-500">~ Réservation ~</h4>
                        <hr/>
                        @auth
                            <div class="form-group">
                                    <input type="text" class="form-control1" name="user" value="{{Auth::user()->name}}" disabled /> 
                            </div>
                        @endauth
                        ...
                        <div class="form-group">
                        	<label>Choisissez votre date de départ
                                <input type="date" class="form-control2" name="depart" required />
                            </label>
                        </div>
                        ...
                        <div class="form-group">
                            <select name="afdville" class="custom-select" required>
                               ...
                               @foreach($data->destinations()->get() as $destination)
                                    @foreach($destination->circuits()->get() as $circuit)
                                            <option value="{{$destination->id}}">{{$destination->label}} - {{$circuit->label}}</option>
                                    @endforeach
                               @endforeach
                            </select>
                        </div>
                        ...
                        <hr/>
                          <div class="form-group">
                              <button type="submit" name="submit" class="more1 btn-block">Réserver <i class="fa fa-paper-plane"></i></button>
                          </div>
                    </form>
                </div>
            </div>
